...
app.get("/api/rooms",async(_,r)=>{try{const{x}=await Promise.resolve({x:null});const{rows}=await query("SELECT id,name,slug,icon FROM rooms ORDER BY created_at");r.json(rows)}catch(e){r.status(500).json({error:"Không thể tải phòng."})}});
...
wss.on("connection",ws=>{
 ...
 ws.on("message",async raw=>{try{const m=JSON.parse(raw);
  if(m.type==="join"){state.userId=m.userId||randomUUID();state.displayName=String(m.displayName||"Khách").trim().slice(0,80);state.roomId=m.roomId;
   ...
   return send(ws,{type:"joined",userId:state.userId,displayName:state.displayName,roomId:state.roomId});
  }
